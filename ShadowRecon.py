...
# NUEVO: Funciones para obtener información precisa de IP, encabezados y tecnologías
import requests
import socket
import json

def get_ip_info(ip):
    # Usa ipinfo.io y fallback a ip-api.com
    try:
        resp = requests.get(f"https://ipinfo.io/{ip}/json", timeout=10)
        if resp.status_code == 200:
            data = resp.json()
            org = data.get("org", "Desconocido")
            asn = org.split()[0] if org and org.startswith("AS") else "N/A"
            pais = data.get("country", "Desconocido")
            loc = data.get("city", "") + " - " + data.get("region", "") + f" ({data.get('loc','')})"
            zona = data.get("timezone", "")
            return org, asn, pais, loc, zona
    except Exception:
        pass
    # Fallback a ip-api.com
    try:
        resp = requests.get(f"http://ip-api.com/json/{ip}?fields=org,as,city,regionName,country,lat,lon,timezone", timeout=10)
        if resp.status_code == 200:
            data = resp.json()
            org = data.get("org", "Desconocido")
            asn = data.get("as", "N/A")
            pais = data.get("country", "Desconocido")
            loc = f"{data.get('city','')} - {data.get('regionName','')} ({data.get('lat','')},{data.get('lon','')})"
            zona = data.get("timezone", "")
            return org, asn, pais, loc, zona
    except Exception:
        pass
    return "Desconocido", "N/A", "Desconocido", " - ()", ""

def get_headers_and_title(url, verify_ssl=True):
    try:
        resp = requests.get(url, timeout=10, verify=verify_ssl, headers=get_random_headers())
        server = resp.headers.get("Server", "Desconocido")
        powered = resp.headers.get("X-Powered-By", "Desconocido")
        # Extraer título
        title = ""
        ...
        return server, powered, title
    except Exception:
        return "Desconocido", "Desconocido", ""

def fingerprint_technologies_precise(url):
    # Usa Wappalyzer API o similar si está disponible, si no, heurística básica
    try:
        # Si tienes clave de Wappalyzer, puedes usar su API aquí
        # Si no, usa heurística simple:
        resp = requests.get(url, timeout=10, verify=False, headers=get_random_headers())
        techs = []
        ...
        if "joomla" in resp.text.lower():
            techs.append("Joomla")
        if "Set-Cookie" in resp.headers and "PHPSESSID" in resp.headers.get("Set-Cookie", ""):
            techs.append("PHP")
        # ...agrega más heurísticas...
        return techs
    except Exception:
        return []

def detect_waf(resp):
    # Heurística simple para WAF
    waf_signatures = [
...
# Ejemplo de uso en el reporte/fingerprint:
def fingerprint_target(domain_or_url):
    # Resuelve IP
    try:
        ip = socket.gethostbyname(domain_or_url.replace("https://", "").replace("http://", "").split("/")[0])
    except Exception:
        ip = None
    org, asn, pais, loc, zona = get_ip_info(ip) if ip else ("Desconocido", "N/A", "Desconocido", " - ()", "")
    # HTTP
    server, powered, title = get_headers_and_title("http://" + domain_or_url, verify_ssl=False)
    # HTTPS
    server_ssl, powered_ssl, title_ssl = get_headers_and_title("https://" + domain_or_url, verify_ssl=False)
    # Tecnologías
    techs = fingerprint_technologies_precise("http://" + domain_or_url)
    # WAF
    try:
        resp = requests.get("http://" + domain_or_url, timeout=10, verify=False, headers=get_random_headers())
        waf = detect_waf(resp)
    except Exception:
        waf = "No detectado"
    # Presentación
    ...
    print(f"Título HTTP         : {title}")
    print(f"Server Header HTTPS : {server_ssl}")
    print(f"X-Powered-By HTTPS  : {powered_ssl}")
    print(f"Título HTTPS        : {title_ssl}")
    print(f"Tecnologías Web     : {techs}")
